...
#define coil_path_error(err, path, format, args...)                          \
    G_STMT_START {                                                           \
        g_set_error(error, COIL_ERROR, COIL_ERROR_PATH,                      \
            "path error (%.*s): " format, path->len, path->str, ##args);     \
    } G_STMT_END
...
void
coil_set_error(GError **error, int code, CoilLocation *location,
        const char *format, ...);

void
coil_set_error_valist(GError **error, int code, CoilLocation *location,
        const char *format, va_list args);
...
void
coil_struct_error(GError **error, CoilObject *obj, const char *format, ...);
